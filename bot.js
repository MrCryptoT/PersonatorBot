...
var AllUsers;
var lastknowntotalmembercount = 0;
var dayssincelastreport = 0; //Only 0 on the first run, then alwayss 1 to be human readable
//var AllUsersCache = [];
//var Memberstoban = [];
//var MembersIDtoban = [];
...
setInterval(() => PrintTotalUsersonServer(), 86400000);//86400000


bot.on('ready', function (evt) {
	if (lastknowntotalmembercount == 0) {
		lastknowntotalmembercount = bot.servers[Servertocheck].member_count;
	}
...
bot.on('guildMemberAdd', function (member) {
	logger.silly(member.id + 'ID joined!');
...
bot.on('guildMemberUpdate', function (oldMember, newMember) {
	...
	if (!(lastguildMemberUpdateusername ==  newMember)){
		...
		logger.silly('User renamed to: ' + newMember.username);
...
function PrintTotalUsersonServer() {
	var currenttotalmembercount = bot.servers[Servertocheck].member_count;
	if (lastknowntotalmembercount == currenttotalmembercount ){
		logger.info("No User Count Change detected");
		if (dayssincelastreport > 0){
			logger.info("Adding 1 to dayssincelastreport");
		 	 dayssincelastreport += 1;
		}
	 }
	if (lastknowntotalmembercount < currenttotalmembercount){
		var diff = currenttotalmembercount - lastknowntotalmembercount
		if (dayssincelastreport == 0){
			var Footertext = "Reports should follow Daily if there was any difference in Total Members!" 
		}else{
			var Footertext = "Exactly " + diff + " more compared to the last Report " + dayssincelastreport + " days ago" 
		}
		sendembed_basic("803985834394648606", 3066993, "User Count Update", "We got " + currenttotalmembercount + " Members in total now!" , Footertext);
	 	logger.info("We got " + currenttotalmembercount + " total Members now!" + Footertext);
		lastknowntotalmembercount = currenttotalmembercount;
		dayssincelastreport = 1;
	}
	 if (lastknowntotalmembercount > currenttotalmembercount){
		var diff = lastknowntotalmembercount - currenttotalmembercount
		if (dayssincelastreport == 0){
			var Footertext = "Reports should follow Daily if there was any difference in Total Members!" 
		}else{
			var Footertext = "Exactly " + diff + " less compared to the last Report " + dayssincelastreport + " days ago" 
		}
		sendembed_basic("803985834394648606", 3066993, "User Count Update", "We got " + currenttotalmembercount + " Members in total now!" , Footertext);
	 	logger.info("We got " + currenttotalmembercount + " total Members now!" + Footertext);
		lastknowntotalmembercount = currenttotalmembercount;
